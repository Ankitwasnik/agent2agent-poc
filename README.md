...
## 9. Where Does a Task Actually Live? InMemoryTaskStore vs. DatabaseTaskStore

Every one of the four agents in this PoC quietly makes the same decision on line one of its `server.py`: `task_store=InMemoryTaskStore()`. It's easy to skim past — one line, sensible default, moving on — but it's worth stopping on, because it answers a question every one of the resilience demos eventually runs into: when you call `tasks/get` an hour later, or restart mid-task, or resubscribe after a dropped connection, *where is that answer actually coming from?*

**`InMemoryTaskStore` is exactly what it sounds like: a Python dictionary.** Keyed by owner, then task ID, living in the same process that's running the agent's HTTP server. No database, no file, no network hop — `tasks/get` is a dict lookup, which is part of why every demo in this PoC feels instant. And here's the detail worth sitting with: there's no eviction. No TTL, no size cap, no background job clearing anything out. A task created the moment the server started is still sitting in that dict, fully retrievable, an hour later, a day later, for as long as the process keeps running. `submit_then_detach.py` and `check_later.py` prove this directly — one process submits a task and exits completely; a second, entirely unrelated process, run however long afterward, calls `tasks/get` cold and gets the right answer, because the dict never knew or cared that the first process existed.

That's a feature for a demo. Left unmodified, it's a liability the moment this stops being one. An agent that runs for weeks accumulates every task it has ever handled, forever, in memory that never shrinks — not really a bug, just "no eviction" outliving its intended lifetime. And there's a quieter second problem: that dictionary lives in exactly one process. Scale the agent horizontally — two replicas behind a load balancer, the ordinary shape of real infrastructure — and a task created on replica A simply doesn't exist as far as replica B's `tasks/get` is concerned. Nothing in the protocol guarantees sticky routing back to the one process holding a given task in memory.

**`DatabaseTaskStore` is the SDK's answer to both problems, and what makes it worth mentioning is how little it costs to adopt.** It implements the exact same `TaskStore` interface — `save`, `get`, `list`, `delete` — just backed by a real, SQLAlchemy-managed database instead of a dict (`a2a-sdk[sql]`; under the hood it takes a SQLAlchemy `AsyncEngine`). Swapping one for the other is, quite literally, one line per agent:

```python
# what every agent in this PoC uses today
task_store = InMemoryTaskStore()

# the production swap — everything else is unchanged
from sqlalchemy.ext.asyncio import create_async_engine
engine = create_async_engine("postgresql+asyncpg://user:pass@host/dbname")
task_store = DatabaseTaskStore(engine)
```

Nothing else moves. Not the `AgentExecutor`, not the routes, not one client script across all four agents and seven resilience demos in this PoC — because none of that code ever talks to a dict or a database directly. It talks to the `TaskStore` interface, and the interface doesn't change. That's the whole argument for designing around an interface, made concrete: the "production-ready" version of this PoC is a config change, not a rewrite, precisely because nothing upstream ever reached past the abstraction to depend on it being in-memory.
